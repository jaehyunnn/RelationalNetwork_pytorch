...
class ChannelAttetion(nn.Module):
    def __init__(self, in_ch=256, reduction=16, use_cuda=True):
        super(ChannelAttetion, self).__init__()
        self.gap = nn.AdaptiveAvgPool2d(1)
        self.att = nn.Sequential(
            nn.Conv2d(in_ch, in_ch//reduction, 1, padding=0),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_ch//reduction, in_ch, 1, padding=0),
            nn.Sigmoid()
        )
...
class net(nn.Module):
    def __init__(self, question_len=11, n_feature=256+2, n_classes=10, use_cuda=True):
        ...
        self.ChannelAttention = ChannelAttetion(in_ch=n_feature, reduction=16)
    ...
    def forward(self, image_batch, question_batch):
        imgFeat = self.ImageFeatrueExtraction(image_batch)
        imgFeat = self.ChannelAttention(imgFeat) # [batch x 256 x 5 x 5]
