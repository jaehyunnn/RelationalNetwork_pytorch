...
class RelationNetwork(nn.Module):
    def __init__(self, n_imgFeat, out_size, qstEmb_size, use_cuda=True):
        ...
        self.att = nn.Sequential(
            nn.Conv2d(5*5*5*5, 5*5*5*5//16, 1),
            nn.ReLU(inplace=True),
            nn.Conv2d(5*5*5*5//16, 5*5*5*5, 1),
            nn.Sigmoid()
        )
    ...
    def forward(self, imgFeat, qstEmb):
        ...
        in_g = torch.cat([o_i,o_j,q], 3).view(b, n_obj*n_obj, (d_obj*2)+d_qst) # [batch x 5*5*5*5 x (256+2)*2+11]

        # Attention
        score = self.att(in_g.mean(dim=2, keepdim=True).unsqueeze(3))
        in_g *= score.squeeze(3)

        in_g = in_g.view(b*n_obj*n_obj, (d_obj*2)+d_qst) # [batch*5*5*5*5 x (256+2)*2+11]
